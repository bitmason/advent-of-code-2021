...
(defun overlap-count (lines) ; # points with overlapping lines
  (let ((m (make-array '(1000 1000)))) ; empirically good enough! ;-)
    ...
    (loop for y below 1000
          sum (loop for x below 1000
                    if (> (aref m x y) 1)
                      sum 1))))

(defun line-orthag-p (coords) ; is horiz or vert?
...
(defun range (a b) ; inclusive (may be asc or desc)
  (if (< a b)
      ...
      (loop for n from a downto b collect n)))

(defun line-points (line) ; list of (x y) covered by line (x1 y1 x2 y2)
  (destructuring-bind (x1 y1 x2 y2) line
    (if (= x1 x2)
        (loop for y in (range y1 y2) ; horiz
              collect (list x1 y))
        (if (= y1 y2)
            (loop for x in (range x1 x2) ; vert
                  collect (list x y1))
            (loop for x in (range x1 x2) ; 45° diag
